...
namespace GuessNumberGame.Models
{
    public class Game
    {
        ...
        public string DifficultyName { get; private set; } = "Custom";

        private bool _hotColdHints;
        ...
        {
            Console.WriteLine("Choose Level: 1) Easy; 2) Medium, 3) Hard, 4) Custom");
            Console.Write("Airchiet (Standartuli : 2): ");
            var level = ReadOption(new[] { "1", "2", "3", "4" }, "2");
            ...
            Console.Write("Gsurt Hot/Cold minishnebebi? (Y/N): ");
            var hotcold = Console.ReadLine()?.Trim().ToLower();
            _hotColdHints = (hotcold == "y" || hotcold == "yes");

            Secret = _rng.Next(Min, Max + 1);
